...
This command is used to construct a NewtonLineSearch algorithm obect
...
residual equation. 
Line search increases the effectiveness of the Newton
method when convergence is slow due to roughness of the residual. 
...
The different line search algorithms use different root finding
methods to obtain $\eta$, a root to the
function $s(\eta)$ defined as:
...
`while` ($\frac{s_n}{s_0}$ &gt; `tol` &amp;&amp; `count` &lt; `maxIter` ) `{`
